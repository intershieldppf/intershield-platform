...
function number(value: unknown): number | null {
  ...
  const normalized = text(value).replace(/[^0-9,.-]/g, "").replace(",", ".");
...
function normalizeImageUrl(value: unknown) {
  ...
  if (url.startsWith("http://http2.mlstatic.com/")) {
    return url.replace("http://http2.mlstatic.com/", "https://http2.mlstatic.com/");
  }
  return url;
}
...
function findMarketplaceHeaderIndex(rows: unknown[][]) {
  return rows.findIndex((row) => {
    ...
    const hasId = headers.some((value) =>
      ["id do anúncios", "id do anúncio", "id do anuncios", "id do anuncio"].includes(value),
    );
    const hasTitle = headers.includes("título") || headers.includes("titulo");
    const hasDescription = headers.includes("descrição") || headers.includes("descricao");
...
export function extractMarketplaceProductsFromWorkbook(workbook: XLSX.WorkBook) {
  ...
  for (const row of sourceRows) {
    const listingId = text(
      readField(row, ["ID do Anúncios", "ID do Anúncio", "ID do Anuncios", "ID do Anuncio"]),
    );
    if (!listingId) continue;

    const existing = grouped.get(listingId);
    const title = text(readField(row, ["Título", "Titulo"])) || null;
    const description = text(readField(row, ["Descrição", "Descricao"])) || null;
    const price = number(readField(row, ["ML Preço", "Preço", "Preco"]));
    const sku = text(readField(row, ["SKU"])) || null;
    const warranty = text(readField(row, ["Tipo de Garantia", "Garantia"])) || null;
...
export function marketplaceImportCoverage(products: MarketplaceProductSource[]) {
  return {
    products: products.length,
    withDescription: products.filter((product) => Boolean(product.description)).length,
